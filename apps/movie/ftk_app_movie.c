...
static Ret _app_movie_selected(void* ctx, int index, const char* path)
{
	char temp[200];
...
static Ret _app_movie_browser(FtkWidget *thiz)
{
	...
	ftk_file_browser_set_choosed_handler(win, _app_movie_selected, thiz);
...
static Ret ftk_movie_on_button_clicked(void* ctx, void* obj)
{
...
static void _app_movie_create_button(FtkWidget *win, int x, int id, const char *icons)
{
	FtkGc gc = {0};
	char temp[100];
	...
	button = ftk_button_create(win, x, 207, 68, 68);
	ftk_widget_set_id(button, id);
	ftk_button_set_clicked_listener(button, ftk_movie_on_button_clicked, win);
...
static Ret ftk_movie_on_shutdown(void* ctx, void* obj)
{
	system("killall mplayer");
...
static Ret ftk_clock_on_prepare_options_menu(void* ctx, FtkWidget* menu_panel)
{
	...
	ftk_menu_item_set_clicked_listener(item, ftk_movie_on_shutdown, ctx);
